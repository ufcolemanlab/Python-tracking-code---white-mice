...
checkFrames = 500 #Sets the number of frames to show before closing frame window
...
#            cv2.line(frame,mid,left,(0,255,0),2)
            ...
            if frameNumber <= roiFrame + checkFrames:
                cv2.imshow(frame_title,frame)
                key = cv2.waitKey(1) & 0xFF
            elif frameNumber > roiFrame + checkFrames:
        ...
        if (len(stimFrames)<3) or (len(stimFrames)<3):
